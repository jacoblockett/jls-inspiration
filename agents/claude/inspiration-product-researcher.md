...
Use `{{INSPIRATION_CLI}}` for research history and any required interface captures. Write only under `.inspiration/` and PACKET_PATH.
...
`{{INSPIRATION_CLI}} history seen <url> --track product --path PROJECT_ROOT`

Record discovery/visit and any capture/inspection/judgment events. Do not casually revisit already exhausted sources.
...
4. record the artifact path in the packet
